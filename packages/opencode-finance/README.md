...
The plugin adds finance commands (`/quote`, `/metrics`, `/filings`, `/watch`, `/portfolio`, `/report-portfolio`, `/report-insiders`, `/financial-government-trading`, `/financial-political-backtest`, `/financial-darkpool-anomaly`, `/report-pdf`, `/market`, `/report`, `/onboard`) and tools (`financial_search`, `portfolio`, `portfolio_report`, `report_insiders`, `report_government_trading`, `financial_political_backtest`, `report_pdf`).
...
`/report-pdf` requires an explicit profile mode: `report`, `government-trading`, `darkpool-anomaly`, or `political-backtest`.
