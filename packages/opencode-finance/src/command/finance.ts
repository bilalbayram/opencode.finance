...
export const FINANCE_SLASH_COMMANDS: FinanceSlashCommand[] = [
  ...
  },
  {
    ...
    template:
      'Run a political-trading event backtest.\n\nExecution rules:\n1) Scope handling:\n   - If "$1" is missing, run portfolio mode using stored holdings.\n   - If "$1" is `portfolio`, run portfolio mode.\n   - Otherwise treat "$1" as ticker mode.\n2) Parse windows from:\n   - ticker mode: "$2"\n   - portfolio mode with explicit keyword: "$2"\n   - missing scope arg portfolio mode: "$1" when it looks like CSV windows\n3) Parse anchor mode (`transaction`, `report`, `both`) from the remaining arg.\n4) Call `financial_political_backtest` with:\n   - `ticker`: include only for ticker mode\n   - `windows`: parsed numeric array when provided\n   - `anchor`: parsed mode when provided\n5) Then provide a concise in-chat summary with:\n   - mode (`ticker` or `portfolio`)\n   - tickers analyzed\n   - event count and benchmark(s)\n   - aggregate hit rate + median return by window\n   - strict-failure notes if any inputs were missing\n6) Point directly to generated artifacts:\n   - `report.md`\n   - `dashboard.md`\n   - `assumptions.json`\n   - raw result tables\n7) After successful markdown artifact generation, ask exactly one `question` prompt:\n   - header: `PDF Export`\n   - question: `Generate a polished PDF report now?`\n   - options:\n     1) `Yes (Recommended)` - Generate PDF in the same output directory.\n     2) `No` - Skip PDF generation.\n   - custom: `false`\n8) If user selects `Yes (Recommended)`, call `report_pdf` with:\n   - `subcommand`: `political-backtest`\n   - `outputRoot`: the output root returned by `financial_political_backtest`\n   - `filename`: `<scope>-<YYYY-MM-DD>.pdf`\n9) If user selects `No`, skip PDF generation.\n10) If `report_pdf` fails, treat the run as failed and report the explicit PDF error.\n\nDo not provide investment advice.',
  },
  {
    name: "market",
